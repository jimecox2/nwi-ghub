...
// GET /api/users?filters[email][$eq]= — Strapi's /users/me omits custom fields
// (customer_id / primary_role), so we resolve them via a filtered users query.
// Returns the normalized extra attributes, or null. The users-permissions
// /users endpoint returns a flat array (no `attributes` wrapper).
export async function getUserByEmail(email, token) {
  if (!email || !token) return null;
  const res = await fetch(
    endpoint(`/api/users?filters[email][$eq]=${encodeURIComponent(email)}`),
    { headers: { Authorization: `Bearer ${token}` } }
  );
  if (!res.ok) return null;
  const data = await res.json();
  ...
  return {
    ...
    email: u.email,
    username: u.username,
    // Strapi field casing varies; try lowercase first, then uppercase.
    customer_id: u.customer_id || u.Customer_id || null,
